...
import { Operator } from "../src/kintone-query-buffer-element";
...
describe("Build Complex Query", () => {
  ...
  it("build and-or condition", () => {
    const query = new KintoneQueryBuilder()
      .where(
        new KintoneQueryExpression().where("a", "<", 1).andWhere("b", "<", 1)
      )
      .orWhere(
        new KintoneQueryExpression().where("c", "<", 1).andWhere("d", "<", 1)
      )
      .build();
