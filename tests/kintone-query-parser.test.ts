...
describe("Parse Monadic Condition", () => {
  ...
  it.each([
    ['field in ("")', 'field in ("")'],
    [
      'フィールド in ("val",100,-1.25,"")',
      'フィールド in ("val",100,-1.25,"")'
    ],
    [
      '     f__1 not   in ( "\\"string"    ,   -0.001   ) ',
      'f__1 not in ("\\"string",-0.001)'
    ],
    [
      "field in (FROM_TODAY(1,DAYS),FROM_TODAY(2,DAYS))",
      "field in (FROM_TODAY(1,DAYS),FROM_TODAY(2,DAYS))"
    ]
...
describe("Parse Combination Condition", () => {
  ...
  it.each([
    ...
    ["(((field=123) and field2<=0))", "(field = 123 and field2 <= 0)"],
    [
      'field like "val" and フィールド in (0,1,2)',
      '(field like "val" and フィールド in (0,1,2))'
    ],
    [
  ...
  it.each([
    ...
    ["(((field=123) or field2<=0))", "(field = 123 or field2 <= 0)"],
    [
      'field like "val" or フィールド in (0,1,2)',
      '(field like "val" or フィールド in (0,1,2))'
    ],
    [
  ...
  it.each([
    ...
    ["(f1=1 or f2=2) and f3=3", "((f1 = 1 or f2 = 2) and f3 = 3)"],
    [
      "((f1=1 or f2=2) and f3=3) or (f4=4 and f5=5)",
      "(((f1 = 1 or f2 = 2) and f3 = 3) or (f4 = 4 and f5 = 5))"
    ]
...
describe("Parse complex query", () => {
  ...
  it.each([
    ...
    [
      "order by field asc,f2 desc,f3 limit 1 offset 500",
      "order by field asc,f2 desc,f3 limit 1 offset 500"
    ],
    ["limit 1 offset 500", "limit 1 offset 500"],
    [
      " order by  field   limit 1  offset  0 ",
      "order by field limit 1 offset 0"
    ]
  ...
  it.each([
    [
      "field=1 order by field asc limit 1",
      "field = 1 order by field asc limit 1"
    ],
    [
